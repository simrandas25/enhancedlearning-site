...
const ClientLayout = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (config?.crisp?.id) {
      ...
      if (config.crisp.onlyShowOnRoutes) {
        ...
        if (config.crisp.onlyShowOnRoutes?.includes(pathname)) {
          Crisp.chat.show();
          Crisp.chat.open();
        } else {
          Crisp.chat.hide();
  ...
  // Set user data in Crisp (live chat support)
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Show tooltips if any JSX elements has these attributes: data-tooltip-id="tooltip" data-tooltip-content="" */}
      <Tooltip
        id="tooltip"
